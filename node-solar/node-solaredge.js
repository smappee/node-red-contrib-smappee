...
function buildRequestUrl (siteId, request, params) {
  ...
  switch (request) {
    ...
    case 'energyDetails':
      // Request for one month with 15-minute values
      params.startTime = moment().subtract(1, 'months').format(API_DATETIME_FORMAT)
    ...
    case 'sensorsData':
      ...
      params.startDate = moment().subtract(1, 'weeks').format(API_DATETIME_FORMAT)
      params.endDate = moment().format(API_DATETIME_FORMAT)
      break
    case 'storageData':
      // Request for one week
      params.startTime = moment().subtract(1, 'weeks').format(API_DATETIME_FORMAT)
      params.endTime = moment().format(API_DATETIME_FORMAT)
      break
  }
